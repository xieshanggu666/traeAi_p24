...
async function migrateWelfareFields() {
  try {
    ...
    try {
      ...
      console.log('coin_records表已存在，跳过');
    }
    ...
    try {
      ...
      console.log('gift_claims表已存在，跳过');
    }
    ...
    try {
      ...
      console.log('task_claims表已存在，跳过');
    }
    ...
    try {
      ...
      console.log('daily_stats表已存在，跳过');
    }

    console.log('✅ 福利功能字段迁移完成');
  } catch (error) {
    console.error('福利功能字段迁移失败:', error.message);
  }
}

module.exports = { migrateWelfareFields };
